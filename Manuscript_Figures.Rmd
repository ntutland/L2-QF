...
Initial_map$`Initial Conditions` <- IC_rst
IC_all <- Initial_map$`Initial Conditions`
Initial_map$`Fire2-Dry` <- F2D_rst
Initial_map$`Fire2-Wet` <- F2W_rst
Initial_map$`Fire5-Dry` <- F5D_rst
Initial_map$`Fire5-Wet` <- F5W_rst
F2D_all <- Initial_map$`Fire2-Dry`
F2W_all <- Initial_map$`Fire2-Wet`
F5D_all <- Initial_map$`Fire5-Dry`
F5W_all <- Initial_map$`Fire5-Wet`
...
```{r}
treelist <- read.csv(here("1.LANDIS-MODEL","LANDIS_run","Treelist_postlandis_cycle0.csv"))

treelist %>%
  ggplot() +
  geom_point(aes(X,Y,color=SPECIES_SYMBOL,size=DBH), shape=1) +
  theme_bw()
#TODO: Just do a single mapcode
```
