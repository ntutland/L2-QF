...
IC_map_dom <- left_join(IC_map_df, IC_dom) %>%
  mutate(SpeciesName = case_when(MapCode %in% c(0,1) ~ "Non-Forest",
                                 TRUE ~ SpeciesName)) %>%
  mutate(SpeciesID = if_else(SpeciesName=="Non-Forest",9,SpeciesID))
...
ggplot() +
  ...
  theme_bw()
ggsave("IC_map.jpg", path=here("Plots"), width = 10, height = 5)
```
...
bar_colors <- colorblind_pal()(8)[c(2,3,1,4,5,6,7,8)]
...
writeVector(mapcodes_poly, here("Plots","sample_mapcode_canopies.shp"))
mapcodes_poly <- mapcodes_poly %>%
  ...
  mutate(SpeciesName = case_when(SPECIES_SYMBOL=="PITA" ~ "Loblolly Pine",
                                 ...
                                 SPECIES_SYMBOL=="LIST2" ~ "Tulip Poplar",
