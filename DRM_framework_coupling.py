...
def main():
    ...
    for i in range(ncycle):
        ii = i + 1
        runTreeQF(VDM,FM,nfuel,qf_options['nx'],qf_options['ny'],qf_options['nz'],ii)      # runs the tree program to create QF inputs
        runQF(i,VDM,qf_options)                           # runs QUIC-Fire
        L=np.array(runCrownScorch(ii,VDM,L2_params,Treelist_params))                  # runs the tree program to create LLM inputs
        L=np.insert(L,0,ii)
        LiveDead.append(L)    
