...
def calculate_QF_vertical_wind_grid(max_topo):
    ...
    zs.append(zo)
    max_topo_adjusted = np.max(
        [max_topo * 3.0, 400.0]
    )  # Covering edge scenario for flat topo
...
def interpolate_array(rasterArray, nodata=None, method="nearest"):  # NEEDS CLEANING UP
    ...
    rasterArray = interpolate.griddata(
...
def remove_shapefile_from_bbox(mask_shp, dom_shp):
    ...
    geoT = (-1.0, 0.0, x2_max, 0.0, -1.0, y2_max)
    msk = np.fliplr(
        np.flipud(geometry_mask(mask_shp.geometry, [msk_yext, msk_xext], geoT))
    )
...
def mask_from_shape(dom, mask, buff):
    ...
    geoT = (-1.0, 0.0, total_bounds[2], 0.0, -1.0, total_bounds[3])

    msk = np.fliplr(
        np.flipud(
            geometry_mask(mask.geometry, [dom.shape[1] * 2, dom.shape[2] * 2], geoT)
        )
    )
...
class AlbersEqualAreaConic:  # Used exclusively to convert Albers  to Lat/Long (Forward) and the Inverse
    ...
    def _q(self, phi):
        ...
        return (1 - self.e2) * (
            np.sin(phi) / (1 - self.e2 * (np.sin(phi)) ** 2)
            - (1.0 / (2 * self.e))
            * np.log((1 - self.e * np.sin(phi)) / (1 + self.e * np.sin(phi)))
        )
...
def fastfuels_access(
    domain_params, out_dir
):  # NEEDS CLEANING UP # DRG, THIS CAN BE ADDED TO THE DOM CLASS ZACH PRODUCED
...
class QF_templates:
    ...
    def print_QFire_Advanced_User_Inputs(self):
        ...
        with open(abs_path, "w") as input_file:
            input_file.write(
                "{{frac_cells_fire|default(0.05)}}			! N/A, fraction of the cells on fire that will launch a firebrand\n"
            )
            input_file.write(
                "{{scl_fac_rad|default(40.0)}}			! N/A, scaling factor of the radius represented by the firebrands launched\n"
            )
            input_file.write(
                "{{ts_firebrands_trj|default(1)}}				! s, time step for the firebrands trajectory calculation\n"
            )
            input_file.write(
                "{{ts_firebrands_launch|default(10)}}				! s, how often to launch firebrands\n"
            )
            input_file.write(
                "{{num_firebrands|default(500)}}			! N/A, number of firebrands distributed over the landing area\n"
            )
            input_file.write(
                "{{fb_fraction_launch|default(20.0)}}			! N/A, FB_FRACTION_LAUNCHED_to_RT_ratio\n"
            )
            input_file.write(
                "{{min_b_value_coef|default(50.0)}}			! N/A, min_b_value_coef\n"
            )
            input_file.write(
                "{{fb_frac_of_max_size|default(0.75)}}			! N/A, fb_frac_of_max_size\n"
            )
            input_file.write(
                "{{germination_delay|default(180)}}				! s, germination_delay\n"
            )
            input_file.write(
                "{{frac_cell_fire|default(10.0)}}				! N/A, fraction of the cell on fire (to scale w)\n"
            )
            input_file.write(
                "{{min_num_ign_firebrands|default(50)}}				! N/A, minimum number of ignitions via firebrands at a point\n"
            )
            input_file.write(
                "{{max_num_ign_firebrands|default(100)}}			! N/A, maximum number of ignitions via firebrands at a point\n"
            )
            input_file.write(
                "{{min_theta_value|default(0.523598)}}		! rad, min_theta_value (pi/6)\n"
            )
    ...
    def print_QFire_Bldg_Advanced_User_Inputs(self):
        ...
        with open(abs_path, "w") as input_file:
            input_file.write(
                "{{quic_urb_blds_flag|default(1)}}				! N/A, flag to convert QUIC-URB buildings to fuel (0 = no, 1 = yes)\n"
            )
            input_file.write(
                "{{fuel_density_blds|default(0.5)}}			! kg/m3, thin fuel density within buildings (if no fuel is specified)\n"
            )
            input_file.write(
                "{{att_coef_blds|default(2.0)}}			! N/A, attenuation coefficient within buildings\n"
            )
            input_file.write(
                "{{surf_rgh_blds|default(0.1)}}	  ! m, surface roughness within buildings\n"
            )
            input_file.write(
                "{{conv_fuel_cnp_winds|default(1)}}				! N/A, flag to convert fuel to canopy for winds (0 = no, 1 = yes)\n"
            )
            input_file.write(
                "{{upd_cnp_winds_fuel|default(1)}}				! N/A, update canopy winds when fuel is consumed\n"
            )
            input_file.write(
                "{{att_coef_fuel|default(1.0)}}			! N/A, attenuation coefficient within fuel\n"
            )
            input_file.write(
                "{{srf_rgh_fuel|default(0.1)}}	  ! m, surface roughness within fuel\n"
            )
            input_file.write("\n")
    ...
    def print_QFire_Plume_Advanced_User_Inputs(self):
        ...
        with open(abs_path, "w") as input_file:
            input_file.write(
                "{{max_plume_ts|default(50000)}}			! N/A, max number of plume at each time step\n"
            )
            input_file.write(
                "{{min_vert_vel_plume|default(0.5)}}			! m/s, minimum vertical velocity of a plume. If wc is below minimum, the plume is eliminated\n"
            )
            input_file.write(
                "{{max_vert_vel_plume|default(10)}}			! m/s, maximum vertical velocity of a plume\n"
            )
            # RRL conversation on 10/20/2021 for how to calc MSR:
            # if 0.1 * ws > 0.5: ; msr = round(0.5/ws, 3) ; else: msr = 0.1
            input_file.write(
                "{{(0.5/wind_speed)|round(3) if 0.1*wind_speed>0.5 else 0.1}}			! N/A, minimum speed ratio (plume vertical velocity/wind speed). If below, the plume is eliminated\n"
            )
            input_file.write(
                "{{brunt_vaisala_frq_squared|default(0)}}			! 1/s^2, brunt vaisala frequency squared\n"
            )
            input_file.write(
                "{{creeping_flag|default(1)}}			! N/A creeping flag: 0 = off, 1 = on\n"
            )
            input_file.write(
                "{{ts_plume_flag|default(0)}}			! N/A, plume time step flag (0 = fixed, 1 = adaptable)\n"
            )
            input_file.write("{{ts_plume|default(1)}}			! s, time step plume\n")
            input_file.write("{{sor_opt|default(1)}}			! s, sor option\n")
            input_file.write(
                "{{alpha_fire_cells|default(2.0)}}			! N/A, alpha 2 for fire cells\n"
            )
            input_file.write(
                "{{frac_ign_ener_2atmos|default(0)}}			! N/A, fraction of ignition energy that goes into en 2 atmos\n"
            )
            input_file.write(
                "{{max_angle_2plumes|default(30)}}			! deg, max angle for merging two plumes\n"
            )
            input_file.write(
                "{{frac_plume_length|default(0.7)}}			! N/A, fraction of a plume length that a point on a second plume can be for merging\n"
            )
            input_file.write(
                "{{max_height_btms_plumes|default(100.0)}}    !Max height bottoms of plumes before removed (<=0 sets to max domain height)\n"
            )
            input_file.write("\n")
            input_file.write("\n")
    ...
    def print_QP_buildout(self):
        ...
        with open(abs_path, "w") as input_file:
            ...
            input_file.write(
                "{{num_veg_cnps|default(0)}}  ! total number of vegitative canopies\n"
            )
    ...
    def print_QUIC_fire(self):
        ...
        with open(abs_path, "w") as input_file:
            input_file.write(
                "{{fire_flag|default(1)}}					! Fire flag: 1 = for fire; 0 = no fire\n"
            )
            input_file.write(
                "{{random|default(-1)}}				! Random number generator: -1: use time and date, any other integer > 0 is used as the seed\n"
            )
            input_file.write("! FIRE TIMES\n")
            input_file.write(
                "{{fire_ign_unix_epoch_time|default(1488794400)}}		! When the fire is ignited in Unix Epoch time (integer seconds since 1970/1/1 00:00:00)\n"
            )
            input_file.write(
                "{{total_time|default(100)}}				! Total simulation time for the fire [s] \n"
            )
            input_file.write(
                "{{time_step|default(1)}}		   		! time step for the fire simulation [s]\n"
            )
            input_file.write(
                "{{ts_wind_field|default(1)}}					! Number of fire time steps done before updating the quic wind field (integer, >= 1)\n"
            )
            input_file.write(
                "{{ts_print_fire_files|default(25)}}					! After how many fire time steps to print out fire-related files (excluding emissions and radiation)\n"
            )
            input_file.write(
                "{{ts_print_wind_files|default(25)}}					! After how many quic updates to print out wind-related files\n"
            )
            input_file.write(
                "{{ts_print_avg_em_rad_files|default(25)}}					! After how many fire time steps to average emissions and radiation\n"
            )
            input_file.write(
                "{{ts_print_avg_wind_files|default(25)}}					! After how many quic updates to print out averaged wind-related files\n"
            )
            input_file.write("! FIRE GRID\n")
            input_file.write(
                "{{num_vertical_layers|default(1)}}					! Number of vertical layers of fire grid cells (integer)\n"
            )
            input_file.write(
                "{{x_fire_grid_ratio|default(1)}}					! x - fire grid ratio = (QUIC-URB cell size)/(fire cell size), integer, can only be >= 1\n"
            )
            input_file.write(
                "{{y_fire_grid_ratio|default(1)}}					! y - fire grid ratio = (QUIC-URB cell size)/(fire cell size), integer, can only be >= 1\n"
            )
            input_file.write(
                "{{vertical_stretching_flag|default(0)}}					! Vertical stretching flag: 0 = uniform dz, 1 = custom\n"
            )
            ...
            input_file.write(
                "{{fire_grid_cell_height|default(1)}}					! Fire grid cell height (m)\n"
            )
            ...
            input_file.write(
                "{{fuel_loc_flag|default(1)}}			! 1 = all fuels in one file, 2 = separate files\n"
            )
            input_file.write(
                "{{fuel_type_flag|default(1)}}			! 1 = stream, 2 = with headers\n"
            )
            input_file.write("! FUEL\n")
            input_file.write(
                "{{fuel_density_flag|default(5)}}					! fuel density flag: 1 = uniform; 2 = provided thru QF_FuelDensity.inp, 3 = Firetech files for quic grid, 4 = Firetech files for different grid (need interpolation)\n"
            )
            input_file.write("{% if fuel_density_flag==1 %}")
            input_file.write("{{fuel_density_val|default(0.7)}}\n")
            input_file.write("{% endif %}")
            input_file.write(
                "{{fuel_moist_flag|default(5)}}					! fuel moisture flag: 1 = uniform; 2 = provided thru QF_FuelMoisture.inp, 3 = Firetech files for quic grid, 4 = Firetech files for different grid (need interpolation)\n"
            )
            ...
            input_file.write(
                "{{ign_loc_flag|default(7)}}					! 1 = rectangle, 2 = square ring, 3 = circular ring, 4 = file (QF_Ignitions.inp), 5 = time-dependent ignitions (QF_IgnitionPattern.inp), 6 = ignite.dat (firetech)\n"
            )
            ...
            input_file.write(
                "{{out_grid_energy_atm|default(0)}}					! Output gridded energy-to-atmosphere (fire grid)\n"
            )
            input_file.write(
                "{{out_cmp_reaction_rate|default(0)}}					! Output compressed array reaction rate (fire grid)\n"
            )
            input_file.write(
                "{{out_cmp_fuel_density|default(1)}}					! Output compressed array fuel density (fire grid)\n"
            )
            input_file.write(
                "{{out_grid_wind|default(1)}}					! Output gridded wind (u,v,w,sigma) (fire grid)\n"
            )
            input_file.write(
                "{{out_grid_qu_wind|default(0)}}					! Output gridded QU winds with fire effects, instantaneous (QUIC-URB grid)\n"
            )
            input_file.write(
                "{{out_grid_qu_avg_wind|default(0)}}					! Output gridded QU winds with fire effects, averaged (QUIC-URB grid)\n"
            )
            input_file.write(
                "{{out_plume_traj|default(0)}}					! Output plume trajectories\n"
            )
            input_file.write(
                "{{out_cmp_fuel_moist|default(0)}}					! Output compressed array fuel moisture (fire grid)\n"
            )
            input_file.write(
                "{{out_vert_mass_burnt|default(0)}}					! Output vertically-integrated % mass burnt (fire grid)\n"
            )
            input_file.write(
                "{{out_grid_plume_locs|default(0)}}					! Output gridded file with plumes locations (QUIC-URB grid)\n"
            )
            input_file.write(
                "{{out_cmp_emis|default(0)}}					! Output compressed array emissions (fire grid)\n"
            )
            input_file.write(
                "{{out_grid_thermal_rad|default(0)}}					! Output gridded thermal radiation (fire grid)\n"
            )
    ...
    def print_QU_buildings(self):
        ...
        with open(abs_path, "w") as input_file:
            ...
            input_file.write(
                "{{wall_rgh_length|default(0.1)}}			!Wall roughness length (m)\n"
            )
            input_file.write("{{num_blds|default(0)}}	!Number of Buildings\n")
            input_file.write(
                "{{num_poly_blds_nodes|default(0)}}	!Number of Polygon Building Nodes\n"
            )
    ...
    def print_QU_fileoptions(self):
        ...
        with open(abs_path, "w") as input_file:
            input_file.write("!QUIC 6.26\n")
            input_file.write(
                "{{out_file_format_flag|default(2)}}   !output data file format flag (1=ascii, 2=binary, 3=both)\n"
            )
            input_file.write(
                "{{uofield_flag|default(0)}}   !flag to write out non-mass conserved initial field (uofield.dat) (1=write,0=no write)\n"
            )
            input_file.write(
                "{{uosensorfield_flag|default(0)}}   !flag to write out the file uosensorfield.dat, the initial sensor velocity field (1=write,0=no write)\n"
            )
            input_file.write(
                "{{qu_staggered_velocity_flag|default(0)}}   !flag to write out the file QU_staggered_velocity.bin used by QUIC-Pressure(1=write,0=no write)\n"
            )
            input_file.write(
                "{{fire_energy_per_timestep|default(0)}}    !Output fire energy per timestep\n"
            )
    ...
    def print_QU_metparams(self):
        ...
        with open(abs_path, "w") as input_file:
            input_file.write("!QUIC 6.26\n")
            input_file.write(
                "{{met_input_flag|default(0)}} !Met input flag (0=QUIC,1=WRF,2=ITT MM5,3=HOTMAC)\n"
            )
            input_file.write(
                "{{num_measu_sites|default(1)}} !Number of measuring sites\n"
            )
            input_file.write(
                "{{max_size_data_points|default(1)}} !Maximum size of data points profiles\n"
            )
    ...
    def print_QU_movingcoords(self):
        ...
        with open(abs_path, "w") as input_file:
            ...
            input_file.write(
                "{{moving_coord_flag|default(0)}}   !Moving coordinates flag (0=no, 1=yes)\n"
            )
            input_file.write(
                "{{ref_bearing_ship_rel|default(0)}}   !Reference bearing of the ship relative to the non-rotated domain (degrees)\n"
            )
            input_file.write(
                "!Time (Unix Time), Ship Speed (m/s), Ship Bearing (deg), Ocean Current Speed (m/s), Ocean Current Direction (deg)\n"
            )
    ...
    def print_QU_simparams(self):
        ...
        with open(abs_path, "w") as input_file:
            ...
            input_file.write(
                "{{vert_stretch_flag|default(3)}} !Vertical stretching flag(0=uniform,1=custom,2=parabolic Z,3=parabolic DZ,4=exponential)\n"
            )
            input_file.write("{{surf_dz|default(1.5)}} !Surface dz (meters)\n")
            input_file.write(
                "{{num_unf_surf_cells|default(4)}} !Number of uniform surface cells\n"
            )
            ...
            input_file.write("{% endif %}")
            input_file.write(
                "{{rooftop_flag|default(2)}} !rooftop flag (0-none, 1-log profile, 2-vortex)\n"
            )
            input_file.write(
                "{{upwind_cavity_flag|default(3)}} !upwind cavity flag (0-none, 1-Rockle, 2-MVP, 3-HMVP)\n"
            )
            input_file.write(
                "{{street_canyon_flag|default(4)}} !street canyon flag (0-none, 1-Roeckle, 2-CPB, 3-exp. param. PKK, 4-Roeckle w/ Fackrel)\n"
            )
            input_file.write(
                "{{street_intsect_flag|default(1)}} !street intersection flag (0-off, 1-on)\n"
            )
            input_file.write(
                "{{wake_flag|default(3)}} !wake flag (0-none, 1-Rockle, 2-Modified Rockle, 3-Area Scaled)\n"
            )
            input_file.write(
                "{{sidewall_flag|default(1)}} !sidewall flag (0-off, 1-on)\n"
            )
            input_file.write(
                "{{canopy_flag|default(2)}} !Canopy flag (1-Cionco w/o wakes, 2-Cionco w/ wakes)\n"
            )
            input_file.write(
                "{{season_flag|default(1)}} !Season flag (1-Summer, 2-Winter, 3-Transition)\n"
            )
            input_file.write(
                "{{max_num_iters|default(10)}} !Maximum number of iterations\n"
            )
            input_file.write(
                "{{omega_relax_val|default(0.9)}} !omega relax value (For sor solver) Must be between 0 and 2\n"
            )
            input_file.write(
                "{{residual_reduc|default(3)}} !Residual Reduction (Orders of Magnitude)\n"
            )
            input_file.write(
                "{{diffusion_alg_flag|default(0)}} !Use Diffusion Algorithm (1 = on)\n"
            )
            input_file.write(
                "{{num_diffusion_iters|default(20)}} !Number of Diffusion iterations\n"
            )
            input_file.write(
                "{{domain_rot_rel_truenorth|default(0)}} !Domain rotation relative to true north (cw = +)\n"
            )
            input_file.write(
                "{{utmx_domain_orign|default(0.0)}}  !UTMX of domain origin (m)\n"
            )
            input_file.write(
                "{{utmy_domain_orign|default(0.0)}}   !UTMY of domain origin (m)\n"
            )
            input_file.write("{{utm_zone|default(1)}} !UTM zone\n")
            input_file.write(
                "{{utm_zone_letter|default(17)}} !UTM zone leter (1=A,2=B,etc.)\n"
            )
            input_file.write("{{quic_cfd_flag|default(0)}} !QUIC-CFD Flag\n")
            input_file.write(
                "{{explo_bld_dmg_flag|default(0)}} !Explosive building damage flag (1 = on)\n"
            )
            input_file.write(
                "{{bld_array_flag|default(0)}} !Building Array Flag (1 = on)\n"
            )
    ...
    def print_sensor1(self):  # Needs to be adjusted for multiple timesteps
        ...
        with open(abs_path, "w") as input_file:
            ...
            input_file.write(
                "{{time_unix_epoch|default(1488794400)}} !Time in unix Epoch Time\n"
            )
            input_file.write("{{site_bdry_layer|default(1)}} !site boundary layer\n")
            input_file.write("{{size_zo|default(0.01)}} !site zo\n")
            input_file.write(
                "{{recp_moninobu_length|default(0.0)}} !reciprocal Monin-Obukhov Length(1/m)\n"
            )
            input_file.write("!height, speed, direction\n")
            input_file.write(
                "{{height|default(10)}} {{wind_speed|default(1.0)}} {{wind_dir|default(270.0)}}\n"
            )
    ...
    def print_topo(self):  #
        ...
        with open(abs_path, "w") as input_file:
            input_file.write(
                '!Relative filepath to topo .dat file (ex:: "../path/to/topo.dat")\n'
            )
            # input_file.write("\"\"\n")
            input_file.write("""{{topo_relpath|default("topo.dat")}}\n""")
            input_file.write(
                "{{topo_flag|default(0)}}              !Topo flag 0:Flat 1:Gaussian Hill 3:Constant slope with flat section 5:Custom .dat\n"
            )
            input_file.write(
                "{{smooth_flag|default(1)}}              !Smoothing Flag\n"
            )
            input_file.write(
                "{{topo_smth_iters|default(100)}}			   ! Smoothing iterations (Topo)\n"
            )
            input_file.write(
                "{{total_startup_iters|default(1000)}}              !Total startup iterations\n"
            )
            input_file.write(
                "{{startup_restart_iters|default(0)}}            !Startup Iterations Restart Period (set to zero for no restart)\n"
            )
            input_file.write(
                "{{preconditioning|default(3)}}              !Preconditioning\n"
            )
...
class IgnitionClass_Rectangular:  # NEEDS TO BE ADDED TO A FULL IGNITION FUNCTION
    ...
    def calc_ignition(self, user_data):  # Done
        ...
        if 225 < wind_dir <= 315:
            self.xmin = (dx * nx) * 0.2
            self.ymin = (dy * ny - pwind_len) / 2.0
            self.xlen = dwind_len
            self.ylen = pwind_len
        elif 135 < wind_dir <= 225:
            self.xmin = (dx * nx - pwind_len) / 2.0
            self.ymin = (dy * ny) * 0.2
            self.xlen = pwind_len
            self.ylen = dwind_len
...
class QF_base:
    ...
    def base_gen_input(self, file_name, data):
        ...
        try:
            ...
            sys.exit("Template Error: {}".format(err.message))
    ...
    def generate_all(self, user_data, **kwargs):
        ...
        if kwargs["output"] is not None:
            user_data["QUIC_fire"]["ts_print_fire_files"] = kwargs["output"][
                "steps_fire"
            ]
            user_data["QUIC_fire"]["ts_print_wind_files"] = kwargs["output"][
                "steps_wind"
            ]
        ...
        if (
            kwargs["fire_grid"] is not None
            and kwargs["fire_grid"]["vertical_stretching"] is not None
        ):

            user_data["QUIC_fire"]["qf_nz"] = kwargs["fire_grid"]["vertical_stretching"]
            user_data["QUIC_fire"]["num_vertical_layers"] = len(
                user_data["QUIC_fire"]["qf_nz"]
            )
    ...
    def print_QU_landuse(self):
        ...
        with open(abs_path, "wb") as input_file:
            ...
            np.array(
                [1.3563156e-19, 2.9410407e-38, 1.3563156e-19, 7.7097618e-33],
                dtype=np.float32,
            ).tofile(input_file)
